...
for (const file of files) {
  ...
  const headerMatch = content.match(/<h1 class="student-hub-title">(.*?)<\/h1>\s*<p class="(?:student-hub-subtitle|text-center.*?)">(.*?)<\/p>/s);
  let title = '', subtitle = '';
  
  if (!headerMatch) {
    const backupMatch = content.match(/<h1 class="student-hub-title">(.*?)<\/h1>/);
    if (backupMatch) {
      title = backupMatch[1];
...
<div class="resource-grid" id="topics-grid">`;
  ...
  content = content.replace(/<h1 class="student-hub-title">.*?<div class="student-hub-grid">/s, newHeader);
