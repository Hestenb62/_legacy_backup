let v=[],L=[],E="all";function Y(){try{v=JSON.parse(localStorage.getItem("hl_completed_levels")||"[]"),L=JSON.parse(localStorage.getItem("hl_bookmarked_levels")||"[]")}catch(e){console.error(e)}}function O(){localStorage.setItem("hl_completed_levels",JSON.stringify(v)),localStorage.setItem("hl_bookmarked_levels",JSON.stringify(L))}function U(){const e=typeof learningLevels<"u"?learningLevels.length:0,t=v.length,s=e?Math.round(t/e*100):0,a=document.getElementById("user-progress-stat");a&&(a.textContent=s+"%")}function j(e){E=e}function V(e){v.includes(e)||v.push(e)}function z(e){const t=v.indexOf(e);t>-1&&v.splice(t,1)}function X(e){L.includes(e)||L.push(e)}function K(e){const t=L.indexOf(e);t>-1&&L.splice(t,1)}function R(e,t,s=!1){if(j(t),document.querySelectorAll(".path-tab").forEach(a=>{a.classList.remove("active"),a.setAttribute("aria-selected","false")}),e&&e.classList.contains("path-tab")&&(e.classList.add("active"),e.setAttribute("aria-selected","true")),document.querySelectorAll(".path-card").forEach(a=>{a.classList.remove("journey-path-active","ring-4","ring-primary/20")}),t!=="all"&&e&&e.classList.contains("path-card")&&e.classList.add("journey-path-active","ring-4","ring-primary/20"),B(),s){const a=document.getElementById("main-content");a&&a.scrollIntoView({behavior:"smooth",block:"start"})}}function B(){const e=document.getElementById("level-search"),t=e?e.value.toLowerCase().trim():"",s=document.querySelectorAll(".level-card"),a=document.getElementById("clear-search");let o=0;a&&(t?a.classList.remove("hidden"):a.classList.add("hidden")),s.forEach(i=>{const h=i.dataset.category,x=i.dataset.id;let g=!1;E==="all"?g=h!=="extra"||x==="test-section":E==="extra"?g=h==="extra"&&x!=="test-section":g=h===E;const S=!t||i.dataset.title&&i.dataset.title.toLowerCase().includes(t)||i.dataset.desc&&i.dataset.desc.toLowerCase().includes(t)||i.dataset.keywords&&i.dataset.keywords.toLowerCase().includes(t);g&&S?(i.classList.remove("hidden"),i.style.display="flex",o++):(i.classList.add("hidden"),i.style.display="none")});const n=document.getElementById("level-grid"),m=document.getElementById("no-results"),l=document.getElementById("results-count"),u=document.getElementById("section-title");n&&m&&(o===0?(n.classList.add("hidden"),m.classList.remove("hidden")):(n.classList.remove("hidden"),m.classList.add("hidden")));const k={all:"Full Journey",elem:"Elementary Path",middle:"Middle School Path",high:"High School Path",extra:"Extra Resources"};u&&(u.textContent=k[E]||"Academic Path"),l&&(l.textContent=`${o} levels available`)}function Q(){const e=document.getElementById("level-search");e&&(e.value=""),R(null,"all")}const q={elem:{icon_bg:"theme-elem-bg",icon_text:"theme-elem-text",label:"Elementary"},middle:{icon_bg:"theme-middle-bg",icon_text:"theme-middle-text",label:"Middle School"},high:{icon_bg:"theme-high-bg",icon_text:"theme-high-text",label:"High School"},extra:{icon_bg:"theme-extra-bg",icon_text:"theme-extra-text",label:"Extra"}};function Z(e){const t=document.getElementById("level-grid");if(!t)return;t.innerHTML=e.map((a,o)=>{const n=q[a.category]||q.elem,m=a.keywords?a.keywords.toLowerCase():"",l=a.title.replace(/'/g,"\\'"),u=a.description.replace(/'/g,"\\'");return`
        ...
        </article>`}).join(""),ee();const s=new IntersectionObserver(a=>{a.forEach(o=>{o.isIntersecting&&(o.target.classList.add("animate-reveal"),s.unobserve(o.target))})},{threshold:.1});document.querySelectorAll(".level-card").forEach(a=>{s.observe(a)}),B()}function ee(){v.forEach(e=>{const t=document.querySelector(`.level-card[data-id="${e}"]`);t&&T(t,!0)}),L.forEach(e=>{const t=document.querySelector(`.level-card[data-id="${e}"] .bookmark-btn`);t&&G(t,!0)}),P()}function T(e,t){const s=e.querySelector(".completion-bar"),a=e.querySelector(".complete-btn"),o=e.querySelector(".level-card-inner");t?(s&&(s.style.width="100%"),a&&(a.classList.add("btn-completed"),a.innerHTML='<i class="fas fa-check text-sm"></i>'),o&&o.classList.add("card-completed")):(s&&(s.style.width="0%"),a&&(a.classList.remove("btn-completed"),a.innerHTML='<i class="fas fa-check text-sm"></i>'),o&&o.classList.remove("card-completed"))}function G(e,t){t?(e.classList.add("btn-bookmarked"),e.innerHTML='<i class="fas fa-star text-sm"></i>'):(e.classList.remove("btn-bookmarked"),e.innerHTML='<i class="far fa-star text-sm"></i>')}function te(e){const t=e.getBoundingClientRect(),s=(t.left+t.width/2)/window.innerWidth,a=(t.top+t.height/2)/window.innerHeight;typeof window.triggerConfetti=="function"&&window.triggerConfetti({x:s,y:a})}function P(){const e=document.getElementById("resume-banner");if(!e)return;const s=Array.from(document.querySelectorAll(".level-card")).find(a=>!v.includes(a.dataset.id));if(s&&v.length>0){const a=s.querySelector("h3").textContent.trim();document.getElementById("next-level-name").textContent=a;const o=s.querySelector("a").href,n=document.getElementById("resume-click-area");n&&(n.onclick=()=>window.location.href=o),e.classList.remove("hidden"),e.classList.add("animate-reveal")}else e.classList.add("hidden")}function ae(){const e=new Date().getHours(),t=document.getElementById("hero-dynamic-greeting");if(!t)return;let s="THE LEARNING ODYSSEY";e<12?s="Good Morning Odyssey":e<18?s="Good Afternoon Journey":s="Good Evening Odyssey",t.textContent=s.toUpperCase()}function se(e,t,s){if("speechSynthesis"in window){window.speechSynthesis.speaking&&window.speechSynthesis.cancel();const a=new SpeechSynthesisUtterance(t+". "+s);window.speechSynthesis.speak(a)}}function oe(e,t){let s;return function(...a){const o=this;clearTimeout(s),s=setTimeout(()=>e.apply(o,a),t)}}function ne(){const e=localStorage.getItem("hl_last_visit"),t=parseInt(localStorage.getItem("hl_streak")||"0"),s=new Date().toDateString(),a=document.getElementById("streak-stat");if(a)if(e===s)a.textContent=t;else if(e){const o=new Date;if(o.setDate(o.getDate()-1),e===o.toDateString()){const n=t+1;localStorage.setItem("hl_streak",n),a.textContent=n,localStorage.setItem("hl_last_visit",s)}else localStorage.setItem("hl_streak",1),a.textContent=1,localStorage.setItem("hl_last_visit",s)}else localStorage.setItem("hl_streak",1),a.textContent=1,localStorage.setItem("hl_last_visit",s)}function ce(e,t){const s=t.closest(".level-card");v.indexOf(e)===-1?(V(e),te(t),T(s,!0)):(z(e),T(s,!1)),O(),U(),P()}function le(e,t){L.indexOf(e)===-1?(X(e),G(t,!0)):(K(e),G(t,!1)),O()}function ie(e){const t=e.closest(".level-card"),s=t.dataset.displayTitle,a=t.dataset.desc,o=t.dataset.icon,n=decodeURIComponent(t.dataset.doc),m=t.dataset.category,l=document.getElementById("doc-modal"),u=document.getElementById("modal-container"),k=l.querySelector(".doc-modal-content"),i={elem:{color:"#14b8a6",bg:"rgba(20, 184, 166, 0.1)",text:"Elementary Path"},middle:{color:"#f59e0b",bg:"rgba(245, 158, 11, 0.1)",text:"Middle School Path"},high:{color:"#e11d48",bg:"rgba(225, 29, 72, 0.1)",text:"High School Path"},extra:{color:"#7c3aed",bg:"rgba(124, 58, 237, 0.1)",text:"Extra Resources"}},h=i[m]||i.elem;u.style.borderTopColor=h.color,document.documentElement.style.setProperty("--color-primary",h.color),document.documentElement.style.setProperty("--color-primary-rgb",m==="elem"?"20, 184, 166":m==="middle"?"245, 158, 11":m==="high"?"225, 29, 72":"124, 58, 237");const x=document.getElementById("modal-icon-container");x.style.backgroundColor=h.bg,document.getElementById("modal-title").textContent=s,document.getElementById("modal-subtitle").textContent=h.text,document.getElementById("modal-icon").className=o,document.getElementById("modal-icon").style.color=h.color,document.getElementById("modal-desc").textContent=a;const g=document.getElementById("modal-docs");let S=[];const D=window.currentSettings&&window.currentSettings.curriculum||"engageny",N=D==="engageny"?"ccss":D,A={"Pre-K":"Pre-K",Kindergarten:"Kindergarten","Grade 1":"1st Grade","Grade 2":"2nd Grade","Grade 3":"3rd Grade","Grade 4":"4th Grade","Grade 5":"5th Grade","Grade 6":"6th Grade","Grade 7":"7th Grade","Grade 8":"8th Grade","Grade 9":"9th Grade","Grade 10":"10th Grade","Grade 11":"11th Grade","Grade 12":"12th Grade"}[s]||s;if(typeof window.curriculumData<"u"&&["math","ela","science","social"].forEach(d=>{const c=window.curriculumData[d],r=c&&c.grades&&c.grades[A]?c.grades[A]:null;if(r){const y=r[N]||r.ccss||r.teks||r.custom;y&&S.push({key:d,name:d==="math"?"Mathematics":d==="ela"?"English Language Arts":c.desc?d.charAt(0).toUpperCase()+d.slice(1):d,data:y})}}),S.length>0){let p='<div class="doc-modal-tab-container">';p+='<div id="modal-tab-slider" class="doc-modal-tab-slider"></div>';let d='<div class="doc-modal-pane-container">';S.forEach((c,r)=>{const y=r===0;p+=`<button type="button" class="modal-tab-pill ${y?"active":""}" data-index="${r}" onclick="switchModalTab(this, ${r})">
                ${c.name}
        ...
        </h4>${p}${d}`,setTimeout(()=>{const c=document.querySelector(".modal-tab-pill");c&&H(c)},50)}else if(n&&n.trim()!==""){const d=new DOMParser().parseFromString(n,"text/html"),c=d.querySelector("h4"),r=d.querySelector("div.space-y-4");if(c&&r){const y=c.textContent,$=Array.from(r.children);let w='<div class="doc-modal-tab-container">';w+='<div id="modal-tab-slider" class="doc-modal-tab-slider"></div>';let C='<div class="doc-modal-pane-container">';$.forEach((f,b)=>{const I=f.querySelector("h5"),F=I?I.textContent:`Module ${b+1}`;let M=f.innerHTML;I&&(M=M.replace(I.outerHTML,""));const _=b===0;w+=`<button type="button" class="modal-tab-pill ${_?"active":""}" data-index="${b}" onclick="switchModalTab(this, ${b})">
                    ${F}
                </button>`;const J=_?"doc-modal-pane active":"doc-modal-pane",W=_?"0s":`${b*.05}s`;C+=`<div class="${J}" data-index="${b}" style="animation-delay: ${W}">
                    <div class="doc-modal-pane-inner">
                        <div class="doc-modal-pane-glow"></div>
                        <div class="doc-modal-pane-content prose-content">
                            ${M}
    ...
    `;const o=document.getElementById("resume-banner");o?o.parentNode.insertBefore(a,o.nextSibling):s.insertBefore(a,s.firstChild);const n=document.getElementById("recommendations-grid");if(!n)return;let m="";t.forEach(l=>{const u=typeof window.learningLevels<"u"?window.learningLevels.find(g=>g.id===l.id):null;if(!u)return;const k=u.link||"#",i=u.title||l.gradeName,h=u.icon||"fas fa-star",x=l.subject.toLowerCase()==="language arts"?"ela":l.subject.toLowerCase();m+=`
        ...
        `}),n.innerHTML=m}function fe(){if(confirm("Are you sure you want to clear your current personalized recommendations?")){localStorage.removeItem("hl_missed_standards");const e=document.getElementById("a11y-focus-recommendations");e&&(e.style.transition="all 0.3s ease-out",e.style.opacity="0",e.style.transform="translateY(15px) scale(0.98)",setTimeout(()=>e.remove(),300))}}window.hl={toggleCompletion:ce,toggleBookmark:le,openDocModal:ie,closeDocModal:de,printCurriculum:re,switchModalTab:me,speakCard:se,clearFocusRecommendations:fe,setCategory:R,resetFilters:Q};document.addEventListener("DOMContentLoaded",()=>{console.log("Learning Odyssey initialized via Vite ES Modules!"),Y(),typeof window.learningLevels<"u"&&(Z(window.learningLevels),B()),ne(),ae(),ue();const e=document.getElementById("level-search");e&&e.addEventListener("input",t=>{const s=document.getElementById("hero-search");s&&(s.value=t.target.value),oe(B,200)()})});
